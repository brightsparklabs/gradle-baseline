...
https://github.com/brightsparklabs/gradle-baseline/actions/workflows/gradle-plugins.yml[image:https://github.com/brightsparklabs/gradle-baseline/actions/workflows/gradle-plugins.yml/badge.svg[Build Status]]
https://plugins.gradle.org/plugin/com.brightsparklabs.gradle.baseline[image:https://img.shields.io/gradle-plugin-portal/v/com.brightsparklabs.gradle.baseline[Gradle Plugin - Baseline]]
https://plugins.gradle.org/plugin/com.brightsparklabs.gradle.baseline-uberjar[image:https://img.shields.io/gradle-plugin-portal/v/com.brightsparklabs.gradle.baseline-uberjar[Gradle Plugin - Baseline Uberjar]]
...
$ ls -l ~/.m2/repository/com/brightsparklabs/gradle-baseline/
----
...
bslBaseline {
    ...
    // NOTE:
    // The following options are useful for development.
    // For example, if you want to test uploading files to a local MinIO instance.
     ...
     * Default: "skip".
     */
    deploy.s3.uploadOverwriteMode = "error"
...
The `bslDeployToS3` task will upload the files specified with the `deploy.s3.filesToUpload` configuration option.
If the files do not exist, the task will do nothing.
If another task creates the files, the `bslDeployToS3` task should depend on it as per standard Gradle configuration.

E.g. Assuming the task `createRelease` creates the necessary files for upload:
...
To upgrade the dependencies of this project, in the base directory (which contains the `build.gradle` file) run the following command:
...
This will list all the gradle dependencies that can be upgraded, and after checking these, dependency versions can be updated manually within the `gradle/libs.versions.toml` file.

[IMPORTANT]
--
. When bumping the `errorprone` plugin, it is important to keep the `error_prone_core` dependency aligned.
  Please refer to `gradle/libs.versions.toml` for details.
. When bumping `spock` you must stay aligned with the version of Groovy that is used by the current gradle version.

E.g. Gradle 8.1.1 uses Groovy 3.0, so you cannot use `org.spockframework:spock-bom:2.3-groovy-4.0` since that specifies Groovy 4.0.
--

=== Upgrading gradle

In order to update the gradle version, you should refer to the relevant documentation provided by gradle (https://docs.gradle.org/current/userguide/upgrading_version_8.html[Example]).
...
This plugin should be tested on a local project before pushing, which can be done with the steps in the <<Testing during development>> section.
...
The Documentation for this JSON Format can be found within the https://github.com/jk1/Gradle-License-Report#allowed-licenses-file[Licence Report Docs].

== Bundled plugins

=== com.brightsparklabs.gradle.baseline-uberjar

Applying this plugin will automatically apply and configure the following plugins:

. https://plugins.gradle.org/plugin/com.diffplug.gradle.spotless[Spotless] for formatting.
...
----
  ...
  // spotless:on
----
. https://plugins.gradle.org/plugin/net.ltgt.errorprone[Error Prone] for static code analysis.
. https://plugins.gradle.org/plugin/com.github.ben-manes.versions[Gradle Versions] for stale dependency checks.
.. `dependencyUpdates` to check for updated dependencies.
. https://plugins.gradle.org/plugin/se.patrikerdes.use-latest-versions[Use Latest Versions] plugins for dependency updates.
...
. https://plugins.gradle.org/plugin/org.owasp.dependencycheck[OWASP] plugin for vulnerability dependency checks.
.. `dependencyCheckAnalyze` to check for vulnerabilities.
. https://plugins.gradle.org/plugin/com.github.jk1.dependency-license-report[License Report] for generating reports about the licenses of dependencies
...
. https://plugins.gradle.org/plugin/com.gradleup.shadow[Shadow] plugin enables the creation of uberjars.
.. `shadowJar` to generate uberjars.
